...
public class TaskServiceImpl implements ITaskService {
	...
	public TaskDTO createTask(TaskPayload payload) {
		...
		if (creator.getRole().equals(Role.HOD)) {
	...
	public List<TaskDTO> getTasksByUser(Long userId) {
		return taskRepository.findByAssignedTo_UserId(userId).stream().map(taskMapper::toDto)
	...
	public List<TaskDTO> getTasksByDepartment(Long deptId) {
		return taskRepository.findByDepartment_DepartmentId(deptId).stream().map(taskMapper::toDto)
				.collect(Collectors.toList());
	}
